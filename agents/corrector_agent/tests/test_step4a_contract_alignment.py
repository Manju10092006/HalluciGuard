...
_CONTRACT_V4_FILES = (TRAIN_FILE, VAL_FILE, TEST_FILE, EDGE_FILE)
_DATASET_PRESENT = all(
    os.path.isfile(fpath) and os.path.getsize(fpath) > 0 for fpath in _CONTRACT_V4_FILES
)


def _skip_without_dataset() -> None:
    """Skip curriculum-data checks when the contract_v4 artifact is not present.

    contract_v4 is generated by the training pipeline (see ``training/``) and is
    not committed to the repo. These tests validate the *contract* of that
    artifact, so they run and must pass in any environment that carries the real
    dataset; in artifact-light clones they are skipped rather than failed.
    """
    if not _DATASET_PRESENT:
        pytest.skip(
            "contract_v4 curriculum dataset not present; build it via the "
            "training pipeline before enabling these checks"
        )


def test_dataset_files_exist_and_non_empty():
    _skip_without_dataset()
    for fpath in _CONTRACT_V4_FILES:
