...
class ProgrammingModelWrapper(BaseModelWrapper):
    """
    Wrapper for code domain models.
    Supports: microsoft/codebert-base (default), microsoft/graphcodebert-base
    """
    
    def __init__(self, model_id: str = "microsoft/codebert-base"):
    ...
    def embed(self, texts: List[str]) -> np.ndarray:
        ...
        if not self._is_available or self._st_model is None:
            logger.warning("Using fallback zero embeddings for programming.")
            return np.zeros((len(texts), self._dim), dtype=np.float32)
        try:
            return self._st_model.encode(texts, normalize_embeddings=True)
        except Exception as e:
            logger.error(f"Error in programming embed: {e}")
            return np.zeros((len(texts), self._dim), dtype=np.float32)
    ...
    def classify(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.error(f"Error in programming classify: {e}")
            return {"LABEL_0": 0.5, "LABEL_1": 0.5}
