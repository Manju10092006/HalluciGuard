...
class CybersecurityModelWrapper(BaseModelWrapper):
    """
    Wrapper for cybersecurity domain models.
    Supports: jackaduma/SecBERT (default)
    """
    
    def __init__(self, model_id: str = "jackaduma/SecBERT"):
    ...
    def _load_internal(self) -> None:
        try:
            ...
            logger.error(f"Failed to load SecBERT embedding model: {e}")
            raise
    ...
    def embed(self, texts: List[str]) -> np.ndarray:
        """Security-domain embeddings."""
        self.ensure_loaded()
        if not self._is_available or self._st_model is None:
            logger.warning("Using fallback zero embeddings for cybersecurity.")
            return np.zeros((len(texts), self._dim), dtype=np.float32)
        try:
            return self._st_model.encode(texts, normalize_embeddings=True)
        except Exception as e:
            logger.error(f"Error in cybersecurity embed: {e}")
            return np.zeros((len(texts), self._dim), dtype=np.float32)
    ...
    def classify(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.error(f"Error in cybersecurity classify: {e}")
            return {"LABEL_0": 0.5, "LABEL_1": 0.5}
    ...
    def extract_entities(self, text: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.error(f"Error in cybersecurity extract_entities: {e}")
            return []
