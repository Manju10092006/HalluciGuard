...
async def _judge_node(state: HalluciGuardState) -> dict[str, Any]:
    ...
    try:
        ...
        correction_attempts = int(state.get("correction_attempt_count", 0))
        ...
        if decision_val == "ACCEPT":
            ...
            if not corr_req or not corr_req.get("claims_to_correct"):
                route = "human_escalation"
                verification_status = "missing_correction_request"
            elif correction_attempts >= max_retries:
                route = "reject"
                verification_status = "correction_attempts_exhausted"
            else:
                route = "corrector"
            ...
            if new_retry_count > max_retries:
                route = "human_escalation"
                verification_status = "verification_retries_exhausted"
            else:
                route = "verifier"
        ...
        return {
            ...
            "trace": add_trace(
                ...
                decision=decision_val,
                severity=severity_val,
                confidence=dumped_judge.get("confidence", 0.0),
                correction_requested=corr_req is not None,
                retry_count=new_retry_count,
            ),
        }
...
def _judge_route(state: HalluciGuardState) -> str:
    ...
    target_route = state.get("route")
    if target_route in ("memory", "corrector", "verifier", "reject", "human_escalation"):
        return target_route
    ...
    if decision == "ACCEPT":
        ...
        corr_req = state.get("correction_request")
        if not corr_req or not corr_req.get("claims_to_correct"):
            return "human_escalation"
        if correction_attempts >= max_retries:
            return "reject"
        return "corrector"
...
async def _corrector_node(state: HalluciGuardState) -> dict[str, Any]:
    from agents.corrector_agent.app.orchestrator import CorrectorOrchestrator
    from agents.corrector_agent.app.models import (
    ...
    try:
        ...
        if not corr_req_data:
            raise RuntimeError("Missing CorrectionRequest payload for Corrector node")

        if isinstance(corr_req_data, dict):
            corr_req = CorrectionRequest.model_validate(corr_req_data)
        else:
            corr_req = corr_req_data

        atomic_claims: list[AtomicClaim] = []
        for c in corr_req.claims_to_correct:
            atomic_claims.append(
                AtomicClaim(
                    ...
                    evidenceIds=[ev.evidence_id for ev in c.evidence],
                )
            )
        for c in corr_req.claims_to_preserve:
            atomic_claims.append(
                AtomicClaim(
                    id=c.claim_id,
                    text=c.claim_text,
        ...
        supporting_ev: list[EvidencePassage] = [
            EvidencePassage(
                id=ev.evidence_id,
                sourceTitle=ev.source,
                passageText=ev.snippet,
                url=ev.url,
                relevanceScore=ev.credibility_score,
            )
            for ev in corr_req.trusted_evidence
        ]
        contradiction_ev: list[EvidencePassage] = [
            EvidencePassage(
                id=ev.evidence_id,
                sourceTitle=ev.source,
                passageText=ev.snippet,
            ...
            for ev in corr_req.contradictory_evidence
        ]

        payload = JudgeVerificationPayload(
            query=corr_req.user_query,
            originalResponse=corr_req.original_response,
            claims=atomic_claims,
            supportingEvidence=supporting_ev,
            contradictionEvidence=contradiction_ev,
            trustScore=0.8,
            correctionInstructions=corr_req.correction_instructions,
        )

        corrector = CorrectorOrchestrator()
        correction_attempts = int(state.get("correction_attempt_count", 0)) + 1
        res = await corrector.executeCorrectionPipeline(payload)
        dumped_res = _dump(res)

        corrected_text = res.finalResponse if hasattr(res, "finalResponse") else dumped_res.get("finalResponse", corr_req.original_response)

        diff_logs = []
        if hasattr(res, "diffLogs") and res.diffLogs:
            for log in res.diffLogs:
                diff_logs.append(_dump(log))
        elif isinstance(dumped_res.get("diffLogs"), list):
            diff_logs = dumped_res.get("diffLogs")

        canonical_corr_result = CorrectionResult(
            original_text=corr_req.original_response,
            corrected_text=corrected_text,
            changed_claims=diff_logs,
            validation_status=ValidationStatus.VALID if getattr(res, "isFullyValid", True) else ValidationStatus.INVALID,
            attempt_count=correction_attempts,
            status=ExecutionStatus.COMPLETED,
        )

        dumped_corr_result = _dump(canonical_corr_result)

        bus = add_bus_message(
            state,
            source_agent="corrector",
            target_agent="supervisor",
            message_type="CORRECTION_RESULT",
            payload={
                "attempt_count": correction_attempts,
                "changed_claims_count": len(diff_logs),
            },
        )

        return {
            "corrector": dumped_corr_result,
            "correction_result": dumped_corr_result,
            "llm_response": corrected_text,
...
async def _reverifier_node(state: HalluciGuardState) -> dict[str, Any]:
    ...
    try:
        VerificationPipeline, SuspiciousClaim, VerifierInputV2 = _verifier_imports()
        corrected_text = state.get("llm_response") or state.get("draft_response", "")
        payload = VerifierInputV2(
            query_id=state.get("request_id") or state.get("execution_id") or str(uuid.uuid4()),
            domain=state.get("domain", "general"),
            suspicious_claims=[SuspiciousClaim(claim_id="c1", text=corrected_text)],
        )

        verifier_res = await VerificationPipeline().verify(payload)
        verifier = _dump(verifier_res)
        ...
        from orchestration.schemas import (
            ...
            VerdictLabel as CanonicalVerdictLabel,
            EntailmentLabel as CanonicalEntailmentLabel,
            ReverificationResult as CanonicalReverificationResult,
            ExecutionStatus,
        )
        ...
        passed = (remaining_contradictions == 0)
        reverification_attempts = int(state.get("reverification_attempt_count", 0)) + 1

        canonical_reverif_result = CanonicalReverificationResult(
            passed=passed,
            verifier_result=canonical_verifier_result,
            remaining_contradictions=remaining_contradictions,
            status=ExecutionStatus.COMPLETED,
        )

        dumped_reverif = _dump(canonical_reverif_result)
        ...
        return {
            "reverification_result": dumped_reverif,
            "verifier_result": _dump(canonical_verifier_result),
            "reverification_attempt_count": reverification_attempts,
            "inter_agent_bus": bus,
            "updated_at": utc_now(),
            "trace": add_trace(
                state,
...
async def _memory_node(state: HalluciGuardState) -> dict[str, Any]:
    ...
    # Memory Safety Check:
    # Memory must ONLY persist when judge decision is ACCEPT, verifier succeeded, and reverification (if present) passed.
    judge_dec = str(state.get("judge_decision", "")).upper()
    verifier_res = state.get("verifier_result") or {}
    verif_status = verifier_res.get("status", "") if isinstance(verifier_res, dict) else str(getattr(verifier_res, "status", ""))
    rev_res = state.get("reverification_result") or {}
    rev_passed = rev_res.get("passed", True) if isinstance(rev_res, dict) else getattr(rev_res, "passed", True)

    if (judge_dec and judge_dec != "ACCEPT") or str(verif_status).lower() == "failed" or not rev_passed:
        memory = {
            "stored": [],
            "count": 0,
            "knowledge_graph": False,
            "vector_memory": False,
            "skipped_reason": "judge_decision_not_accepted",
        }
        bus = add_bus_message(
            state,
            source_agent="memory",
            ...
            payload={"stored_count": 0, "status": "skipped", "reason": "judge_decision_not_accepted"},
        )
        return {
            "memory": memory,
